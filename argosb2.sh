...
insuuid(){
    if [ -z "$uuid" ]; then
        if [ -e "$HOME/agsb/agsb/sing-box" ]; then
            uuid=$("$HOME/agsb/agsb/sing-box" generate uuid)
        elif [ -e "$HOME/agsb/agsb/xray" ]; then
            uuid=$("$HOME/agsb/agsb/xray" uuid)
        else
            # 如果两个内核都没有，生成一个随机UUID
            uuid=$(cat /proc/sys/kernel/random/uuid)
        fi
    fi
    echo "$uuid" > "$HOME/agsb/agsb/uuid"
    echo "UUID密码：$uuid"
}
...
installxray(){
    ...
    if [ ! -e "$HOME/agsb/agsb/xray" ]; then
        ...
        download_and_verify "$xray_url" "$HOME/agsb/agsb/xray" "ELF 64-bit executable"
        
        # 验证Xray是否可运行
        if ! "$HOME/agsb/agsb/xray" version >/dev/null 2>&1; then
            error_exit "Xray内核无法运行，请检查系统兼容性"
        fi
        
        sbcore=$("$HOME/agsb/agsb/xray" version 2>/dev/null | awk '/^Xray/{print $2}')
        echo "已安装Xray正式版内核：$sbcore"
    fi
    cat > "$HOME/agsb/agsb/xr.json" <<EOF
{
  "log": {
    "access": "/dev/null",
    "error": "/dev/null",
...
EOF
    ...
    if [ -n "$xhp" ] || [ -n "$vlp" ]; then
        ...
        echo "$ym_vl_re" > "$HOME/agsb/agsb/ym_vl_re"
        echo "Reality域名：$ym_vl_re"
        mkdir -p "$HOME/agsb/agsb/xrk"
        if [ ! -e "$HOME/agsb/agsb/xrk/private_key" ]; then
            key_pair=$("$HOME/agsb/agsb/xray" x25519)
            private_key=$(echo "$key_pair" | head -1 | awk '{print $3}')
            public_key=$(echo "$key_pair" | tail -n 1 | awk '{print $3}')
            short_id=$(date +%s%N | sha256sum | cut -c 1-8)
            echo "$private_key" > "$HOME/agsb/agsb/xrk/private_key"
            echo "$public_key" > "$HOME/agsb/agsb/xrk/public_key"
            echo "$short_id" > "$HOME/agsb/agsb/xrk/short_id"
        fi
        private_key_x=$(cat "$HOME/agsb/agsb/xrk/private_key")
        public_key_x=$(cat "$HOME/agsb/agsb/xrk/public_key")
        short_id_x=$(cat "$HOME/agsb/agsb/xrk/short_id")
    fi
    if [ -n "$xhp" ]; then
        xhp=xhpt
        if [ -z "$port_xh" ]; then
        ...
        echo "$port_xh" > "$HOME/agsb/agsb/port_xh"
...
EOF
    ...
    if [ -n "$vlp" ]; then
        ...
        echo "$port_vl_re" > "$HOME/agsb/agsb/port_vl_re"
...
installsb(){
    ...
    if [ ! -e "$HOME/agsb/agsb/sing-box" ]; then
        ...
        download_and_verify "$sb_url" "$HOME/agsb/agsb/sing-box" "ELF 64-bit executable"
        
        # 验证Sing-box是否可运行
        if ! "$HOME/agsb/agsb/sing-box" version >/dev/null 2>&1; then
            error_exit "Sing-box内核无法运行，请检查系统兼容性"
        fi
        
        sbcore=$("$HOME/agsb/agsb/sing-box" version 2>/dev/null | awk '/version/{print $NF}')
        echo "已安装Sing-box正式版内核：$sbcore"
    fi
    cat > "$HOME/agsb/agsb/sb.json" <<EOF
{
"log": {
    "disabled": false,
    "level": "info",
...
EOF
    insuuid
    command -v openssl >/dev/null 2>&1 && openssl ecparam -genkey -name prime256v1 -out "$HOME/agsb/agsb/private.key" >/dev/null 2>&1
    command -v openssl >/dev/null 2>&1 && openssl req -new -x509 -days 36500 -key "$HOME/agsb/agsb/private.key" -out "$HOME/agsb/agsb/cert.pem" -subj "/CN=www.bing.com" >/dev/null 2>&1
    if [ ! -f "$HOME/agsb/agsb/private.key" ]; then
        curl -Lso "$HOME/agsb/agsb/private.key" https://github.com/zzzhhh1/ArgoSB/releases/download/argosbx/private.key
        curl -Lso "$HOME/agsb/agsb/cert.pem" https://github.com/zzzhhh1/ArgoSB/releases/download/argosbx/cert.pem
    fi
    if [ -n "$hyp" ]; then
        hyp=hypt
        if [ -z "$port_hy2" ]; then
        ...
        echo "$port_hy2" > "$HOME/agsb/agsb/port_hy2"
        ...
        cat >> "$HOME/agsb/agsb/sb.json" <<EOF
    {
        ...
        "tls": {
            ...
            "certificate_path": "$HOME/agsb/agsb/cert.pem",
            "key_path": "$HOME/agsb/agsb/private.key"
        }
    },
EOF
    else
    ...
    if [ -n "$tup" ]; then
        ...
        echo "$port_tu" > "$HOME/agsb/agsb/port_tu"
        echo "Tuic端口：$port_tu"
        cat >> "$HOME/agsb/agsb/sb.json" <<EOF
        {
            "type":"tuic",
...
EOF
    ...
    if [ -n "$anp" ]; then
        ...
        echo "$port_an" > "$HOME/agsb/agsb/port_an"
...
xrsbvm(){
    if [ -n "$vmp" ]; then
        ...
        echo "$port_vm_ws" > "$HOME/agsb/agsb/port_vm_ws"
...
xrsbout(){
    # 修复Xray启动部分的语法错误
    if [ -e "$HOME/agsb/agsb/xray" ]; then
        sed -i '${s/,\s*$//}' "$HOME/agsb/agsb/xr.json"
        cat >> "$HOME/agsb/agsb/xr.json" <<EOF
  ],
  "outbounds": [
    {
...
EOF
        echo "启动Xray服务..."
        # 修复语法错误：正确的后台启动方式
        if ! nohup "$HOME/agsb/agsb/xray" run -c "$HOME/agsb/agsb/xr.json" >/dev/null 2>&1 & then
            error_exit "Xray服务启动失败"
        fi
        # 检查Xray是否成功启动
        sleep 2
        if ! pgrep -f "$HOME/agsb/agsb/xray" >/dev/null 2>&1; then
    ...
    # 修复Sing-box启动部分的语法错误
    if [ -e "$HOME/agsb/agsb/sing-box" ]; then
        sed -i '${s/,\s*$//}' "$HOME/agsb/agsb/sb.json"
        cat >> "$HOME/agsb/agsb/sb.json" <<EOF
],
"outbounds": [
{
...
EOF
        echo "启动Sing-box服务..."
        # 修复语法错误：正确的后台启动方式
        if ! nohup "$HOME/agsb/agsb/sing-box" run -c "$HOME/agsb/agsb/sb.json" >/dev/null 2>&1 & then
            error_exit "Sing-box服务启动失败"
        fi
        # 检查Sing-box是否成功启动
        sleep 2
        if ! pgrep -f "$HOME/agsb/agsb/sing-box" >/dev/null 2>&1; then
...
ins(){
    ...
    if [ -n "$argo" ] && [ -n "$vmag" ]; then
        ...
        if [ ! -e "$HOME/agsb/agsb/cloudflared" ]; then
            ...
            download_and_verify "$cloudflared_url" "$HOME/agsb/agsb/cloudflared" "ELF 64-bit executable"

            # 验证Cloudflared是否可运行
            if ! "$HOME/agsb/agsb/cloudflared" version >/dev/null 2>&1; then
        ...
        if [ -n "${ARGO_DOMAIN}" ] && [ -n "${ARGO_AUTH}" ]; then
            ...
            echo "启动固定Argo隧道..."
            # 修复启动命令语法
            if ! nohup "$HOME/agsb/agsb/cloudflared" tunnel --no-autoupdate --edge-ip-version auto --protocol http2 run --token "${ARGO_AUTH}" >/dev/null 2>&1 & then
                error_exit "固定Argo隧道启动失败"
            fi
            echo "${ARGO_DOMAIN}" > "$HOME/agsb/agsb/sbargoym.log"
            echo "${ARGO_AUTH}" > "$HOME/agsb/agsb/sbargotoken.log"
        else
            name='临时'
            echo "启动临时Argo隧道..."
            # 修复启动命令语法
            if ! nohup "$HOME/agsb/agsb/cloudflared" tunnel --url http://localhost:"${port_vm_ws}" --edge-ip-version auto --no-autoupdate --protocol http2 > "$HOME/agsb/agsb/argo.log" 2>&1 & then
        ...
        sleep 8
        if [ -n "${ARGO_DOMAIN}" ] && [ -n "${ARGO_AUTH}" ]; then
            argodomain=$(cat "$HOME/agsb/agsb/sbargoym.log" 2>/dev/null)
        else
            argodomain=$(grep -a trycloudflare.com "$HOME/agsb/agsb/argo.log" 2>/dev/null | awk 'NR==2{print}' | awk -F// '{print $2}' | awk '{print $1}')
        fi
    ...
    if find /proc/*/exe -type l 2>/dev/null | grep -E '/proc/[0-9]+/exe' | xargs -r readlink 2>/dev/null | grep -Eq 'agsb/(s|x)' || pgrep -f 'agsb/(s|x)' >/dev/null 2>&1; then
        ...
        if find /proc/*/exe -type l 2>/dev/null | grep -E '/proc/[0-9]+/exe' | xargs -r readlink 2>/dev/null | grep -q 'agsb/s' || pgrep -f 'agsb/s' >/dev/null 2>&1; then
            echo '@reboot /bin/sh -c "nohup $HOME/agsb/agsb/sing-box run -c $HOME/agsb/agsb/sb.json >/dev/null 2>&1 &"' >> /tmp/crontab.tmp
        fi
        if find /proc/*/exe -type l 2>/dev/null | grep -E '/proc/[0-9]+/exe' | xargs -r readlink 2>/dev/null | grep -q 'agsb/x' || pgrep -f 'agsb/x' >/dev/null 2>&1; then
            echo '@reboot /bin/sh -c "nohup $HOME/agsb/agsb/xray run -c $HOME/agsb/agsb/xr.json >/dev/null 2>&1 &"' >> /tmp/crontab.tmp
        fi
        sed -i '/agsb\/cloudflared/d' /tmp/crontab.tmp
        if [ -n "$argo" ] && [ -n "$vmag" ]; then
            if [ -n "${ARGO_DOMAIN}" ] && [ -n "${ARGO_AUTH}" ]; then
                echo '@reboot /bin/sh -c "nohup $HOME/agsb/agsb/cloudflared tunnel --no-autoupdate --edge-ip-version auto --protocol http2 run --token $(cat $HOME/agsb/agsb/sbargotoken.log 2>/dev/null) >/dev/null 2>&1 &"' >> /tmp/crontab.tmp
            else
                if [ -e "$HOME/agsb/agsb/xray" ]; then
                    echo '@reboot /bin/sh -c "nohup $HOME/agsb/agsb/cloudflared tunnel --url http://localhost:$(grep -A2 vmess-xr $HOME/agsb/agsb/xr.json | tail -1 | tr -cd 0-9) --edge-ip-version auto --no-autoupdate --protocol http2 > $HOME/agsb/agsb/argo.log 2>&1 &"' >> /tmp/crontab.tmp
                else
                    echo '@reboot /bin/sh -c "nohup $HOME/agsb/agsb/cloudflared tunnel --url http://localhost:$(grep -A2 vmess-sb $HOME/agsb/agsb/sb.json | tail -1 | tr -cd 0-9) --edge-ip-version auto --no-autoupdate --protocol http2 > $HOME/agsb/agsb/argo.log 2>&1 &"' >> /tmp/crontab.tmp
                fi
            fi
        fi
...
cip(){
    ipbest(){
        ...
        if echo "$serip" | grep -q ':'; then
            server_ip="[$serip]"
            echo "$server_ip" > "$HOME/agsb/agsb/server_ip.log"
        else
            server_ip="$serip"
            echo "$server_ip" > "$HOME/agsb/agsb/server_ip.log"
        fi
    ...
    rm -rf "$HOME/agsb/agsb/jh.txt"
    uuid=$(cat "$HOME/agsb/agsb/uuid")
    server_ip=$(cat "$HOME/agsb/agsb/server_ip.log")
    echo "*********************************************************"
    echo "*********************************************************"
    echo "ArgoSB脚本输出节点配置如下："
    echo
    if [ -f "$HOME/agsb/agsb/port_xh" ] || [ -f "$HOME/agsb/agsb/port_vl_re" ]; then
        ym_vl_re=$(cat "$HOME/agsb/agsb/ym_vl_re")
        private_key_x=$(cat "$HOME/agsb/agsb/xrk/private_key")
        public_key_x=$(cat "$HOME/agsb/agsb/xrk/public_key")
        short_id_x=$(cat "$HOME/agsb/agsb/xrk/short_id")
    fi
    if [ -f "$HOME/agsb/agsb/port_xh" ]; then
        echo "【 vless-xhttp-reality 】节点信息如下："
        port_xh=$(cat "$HOME/agsb/agsb/port_xh")
        ...
        echo "$vl_xh_link" >> "$HOME/agsb/agsb/jh.txt"
        echo "$vl_xh_link"
        echo
    fi
    if [ -f "$HOME/agsb/agsb/port_vl_re" ]; then
        echo "【 vless-reality-vision 】节点信息如下："
        port_vl_re=$(cat "$HOME/agsb/agsb/port_vl_re")
        ...
        echo "$vl_link" >> "$HOME/agsb/agsb/jh.txt"
        echo "$vl_link"
        echo
    fi
    if [ -f "$HOME/agsb/agsb/port_vm_ws" ]; then
        echo "【 vmess-ws 】节点信息如下："
        port_vm_ws=$(cat "$HOME/agsb/agsb/port_vm_ws")
        ...
        echo "$vm_link" >> "$HOME/agsb/agsb/jh.txt"
        echo "$vm_link"
        echo
    fi
    if [ -f "$HOME/agsb/agsb/port_an" ]; then
        echo "【 AnyTLS 】节点信息如下："
        port_an=$(cat "$HOME/agsb/agsb/port_an")
        an_link="anytls://$uuid@$server_ip:$port_an?insecure=1#anytls-$hostname"
        echo "$an_link" >> "$HOME/agsb/agsb/jh.txt"
        echo "$an_link"
        echo
    fi
    if [ -f "$HOME/agsb/agsb/port_hy2" ]; then
        echo "【 Hysteria2 】节点信息如下："
        port_hy2=$(cat "$HOME/agsb/agsb/port_hy2")
        hy2_link="hysteria2://$uuid@$server_ip:$port_hy2?security=tls&alpn=h3&insecure=1&sni=www.bing.com#hy2-$hostname"
        echo "$hy2_link" >> "$HOME/agsb/agsb/jh.txt"
        echo "$hy2_link"
        echo
    fi
    if [ -f "$HOME/agsb/agsb/port_tu" ]; then
        echo "【 Tuic 】节点信息如下："
        port_tu=$(cat "$HOME/agsb/agsb/port_tu")
        ...
        echo "$tuic5_link" >> "$HOME/agsb/agsb/jh.txt"
        echo "$tuic5_link"
        echo
    fi
    argodomain=$(cat "$HOME/agsb/agsb/sbargoym.log" 2>/dev/null)
    [ -z "$argodomain" ] && argodomain=$(grep -a trycloudflare.com "$HOME/agsb/agsb/argo.log" 2>/dev/null | awk 'NR==2{print}' | awk -F// '{print $2}' | awk '{print $1}')
    if [ -n "$argodomain" ]; then
        ...
        echo "$vmatls_link1" >> "$HOME/agsb/agsb/jh.txt"
        ...
        echo "$vmatls_link2" >> "$HOME/agsb/agsb/jh.txt"
        ...
        echo "$vmatls_link3" >> "$HOME/agsb/agsb/jh.txt"
        ...
        echo "$vmatls_link4" >> "$HOME/agsb/agsb/jh.txt"
        ...
        echo "$vmatls_link5" >> "$HOME/agsb/agsb/jh.txt"
        ...
        echo "$vmatls_link6" >> "$HOME/agsb/agsb/jh.txt"
        ...
        echo "$vma_link7" >> "$HOME/agsb/agsb/jh.txt"
        ...
        echo "$vma_link8" >> "$HOME/agsb/agsb/jh.txt"
        ...
        echo "$vma_link9" >> "$HOME/agsb/agsb/jh.txt"
        ...
        echo "$vma_link10" >> "$HOME/agsb/agsb/jh.txt"
        ...
        echo "$vma_link11" >> "$HOME/agsb/agsb/jh.txt"
        ...
        echo "$vma_link12" >> "$HOME/agsb/agsb/jh.txt"
        ...
        echo "$vma_link13" >> "$HOME/agsb/agsb/jh.txt"
        sbtk=$(cat "$HOME/agsb/agsb/sbargotoken.log" 2>/dev/null)
        if [ -n "$sbtk" ]; then
    ...
    echo -e "$argoshow"
    echo "---------------------------------------------------------"
    echo "聚合节点信息，请查看$HOME/agsb/agsb/jh.txt文件或者运行cat $HOME/agsb/agsb/jh.txt进行复制"
    echo "---------------------------------------------------------"
...
if [ "$1" = "del" ]; then
    ...
    crontab /tmp/crontab.tmp 2>/dev/null
    rm /tmp/crontab.tmp
    rm -rf "$HOME/agsb/agsb" "$HOME/bin/agsb"
    echo "卸载完成"
    exit 0
elif [ "$1" = "list" ]; then
    cip
